 ...
 * package at http://rsb.info.nih.gov/ij <blockquote> ImageJ is being developed
 * at the National Institutes of Health by an employee of the Federal Government
 * in the course of his official duties. Pursuant to Title 17, Section 105 of
 * the United States Code, this software is not subject to copyright protection
 * and is in the public domain. ImageJ is an experimental system. NIH assumes no
 * responsibility whatsoever for its use by other parties, and makes no
 * guarantees, expressed or implied, about its quality, reliability, or any
 * other characteristic. </blockquote> The ImageJ AviWriter class was based on
 * the FileAvi class written by William Gandler. That FileAvi class is part of
 * Matthew J. McAuliffe's MIPAV program, available from
 * http://mipav.cit.nih.gov/, which also appears to be in the public domain.
 * <p>
 * I owe my MJPG understanding to the jpegtoavi program.
 * http://sourceforge.net/projects/jpegtoavi/
 * <p>
 * Random list of codecs
 * http://www.oltenia.ro/download/pub/windows/media/video/tools
 * /GSpot/gspot22/GSpot22.dat
 * <p>
...
public class AviWriter {
	...
	/**
	 * Size of the frame data in bytes. Only applicable to DIB AVI. Each DIB
	 * frame submitted is compared to this value to ensure proper data.
	 */
	...
	public void setSamplesPerSecond(int i) {
		if (i < 1)
			throw new IllegalArgumentException(
					"Samples/Second must be greater than 0");
	...
	public void setFramesPerSecond(int i, int j) {
		if (i < 1 || j < 1)
			throw new IllegalArgumentException(
					"frames/sec must be greater than 0");
		m_iFrames = i;
		m_iPerSecond = j;
	}
	...
	public AviWriter(final File oOutputfile, final int iAudChannels)
			throws IOException {
		this(oOutputfile, iAudChannels, false);
	}

	/**
	 * Write MJPG encoded frames with default quality, or uncompressed RGB
	 * device-independent bitmap (DIB) frames.
	 */
	public AviWriter(final File oOutputfile, final int iAudChannels,
			final boolean blnEncodeMjpg) throws IOException {

		if (blnEncodeMjpg) {
			if (!CAN_ENCODE_JPEG)
				throw new UnsupportedOperationException(
						"Unable to encode 'jpeg' on this platform.");
	 ...
	 * 
	 * @param fltMjpgQuality
	 *            quality from 0 (lowest) to 1 (highest).
	 */
	public AviWriter(final File oOutputfile, final int iAudChannels,
			final float fltMjpgQuality) throws IOException {
		if (!CAN_ENCODE_JPEG)
			throw new UnsupportedOperationException(
					"Unable to encode 'jpeg' on this platform.");
	...
	private void InitAVIWriter(final File oOutputfile, final int iAudChannels)
			throws IOException {
	...
	/**
	 * Assumes width/height is correct. abData should either be DIB data (BGR
	 * rows inverted and widths padded to 4 byte boundaries), or JPEG data with
	 * the 'JFIF' header text changed to 'AVI1'.
	 */
	public void writeFrame(byte[] abData) throws IOException {
		if (raFile == null)
			throw new IOException("Avi file is closed");
		// only keep track of frame data size if it's DIB. They should all be
		// the same
		if (!m_blnMJPG) {
			if (m_iFrameByteSize < 0)
				...
				throw new IllegalArgumentException(
						"Frame data size is not consistent");
		}
	...
	public void writeAudio(AudioInputStream oData) throws IOException {
		...
		if (fmt.getSampleSizeInBits() != m_iBytesPerSample * 8)
		...
		idxentry.dwOffset = (int) (raFile.getFilePointer() - (LIST_movi
				.getStart() + 4));
		...
		int remaint = (int) (4 - ((raFile.getFilePointer() - (data_size
				.getStart() + 4)) % 4)) % 4;
		while (remaint > 0) {
			raFile.write(0);
			remaint--;
	...
	private void writeStreamDataChunk(byte[] abData, boolean blnIsVideo)
			throws IOException {
		...
		if (blnIsVideo) { // if video
			...
			idxentry.dwFlags = AVIOLDINDEX.AVIIF_KEYFRAME; // Write the flags -
															// select
															// AVIIF_KEYFRAME
															// AVIIF_KEYFRAME
															// 0x00000010L
			...
			if (abData.length % m_iBytesPerSample != 0
				...
				throw new IllegalArgumentException(
						"Half an audio sample can't be processed.");

			idxentry.dwChunkId = AVIstruct.string2int("01wb");
			idxentry.dwFlags = 0;

			m_dblSampleCount += (double) abData.length / m_iBytesPerSample
					/ m_iChannels;
	...
	public void close() throws IOException {
		...
		if (m_iFrames < 1 || m_iPerSecond < 1)
			throw new IllegalStateException(
					"Must set frames/second before closing avi");
		if (m_iChannels > 0 && m_iSamplesPerSecond < 1)
			throw new IllegalStateException(
					"Must set samples/second before closing avi");
		if (m_iWidth < 0 || m_iHeight < 0)
			throw new IllegalStateException(
					"Must set dimentions before closing avi");
		// //////////////////////////////////////////////////////////////////////
		...
		// avih.cb = 0x38; // the length of the avih sub-CHUNK (38H) not
		// including the
		// the first 8 bytes for avihSignature and the length
		avih.dwMicroSecPerFrame = (int) ((m_iPerSecond / (double) m_iFrames) * 1.0e6);
		avih.dwMaxBytesPerSec = 0; // (maximum data rate of the file in bytes
									// per second)
		...
		if (m_iChannels > 0)
			avih.dwStreams = 2; // number of streams in the file - here 1 video
								// and zero audio.
		else
			avih.dwStreams = 1; // number of streams in the file - here 1 video
								// and zero audio.
		avih.dwSuggestedBufferSize = 0; // Suggested buffer size for reading the
										// file.
										// Generally, this size should be large
										// enough to contain the largest
										// chunk in the file.
										// dwSuggestedBufferSize - Suggested
		...
		// avih.dwReserved1 = 0; // Microsoft says to set the following 4 values
		// to 0.
		// avih.dwReserved2 = 0; //
		// avih.dwReserved3 = 0; //
		// avih.dwReserved4 = 0; //
		...
		strh_vid.fccType = AVIstruct.string2int("vids"); // the type of data
															// stream - here
															// vids for video
															// stream
		...
		strh_vid.dwLength = m_iFrameCount; // playing time of AVI file as
											// defined by scale and rate
		// Set equal to the number of frames
		strh_vid.dwSuggestedBufferSize = 0; // Suggested buffer size for reading
											// the stream.
											// Typically, this contains a value
											// corresponding to the largest
											// chunk
											// in a stream.
		strh_vid.dwQuality = -1; // encoding quality given by an integer between
									// 0 and 10,000. If set to -1, drivers use
									// the default
									// quality value.
		...
		bif.biHeight = m_iHeight; // image height in pixels. If height is
									// positive,
									// the bitmap is a bottom up DIB and its
									// origin is in the lower left corner. If
									// height is negative, the bitmap is a
									// top-down DIB and its origin is the upper
									// left corner. This negative sign feature
									// is supported by the Windows Media Player,
									// but it is not
									// supported by PowerPoint.
		// bif.biPlanes = 1; // biPlanes - number of color planes in which the
		// data is stored
		...
		bif.biYPelsPerMeter = 0; // vertical resolution in pixels
									// per meter
		bif.biClrUsed = 0; //
		bif.biClrImportant = 0; // biClrImportant - specifies that the first x
								// colors of the color table
								// are important to the DIB. If the rest of the
								// colors are not available,
								// the image still retains its meaning in an
								// acceptable manner. When this
								// field is set to zero, all the colors are
								// important, or, rather, their
		...
		if (m_iChannels > 0) {
			...
			strh_aud.fccType = AVIstruct.string2int("auds"); // Write the type
																// of data
																// stream - here
																// auds for
																// audio stream
			...
			strh_aud.dwScale = 1;
			strh_aud.dwRate = m_iSamplesPerSecond; // sample rate for audio
													// streams
			strh_aud.dwStart = 0; // this field is usually set to zero
			strh_aud.dwLength = (int) m_dblSampleCount; // playing time of AVI
														// file as defined by
														// scale and rate
			// Set equal to the number of audio samples in file?
			strh_aud.dwSuggestedBufferSize = 0; // Suggested buffer size for
												// reading the stream.
												// Typically, this contains a
												// value corresponding to the
												// largest chunk
												// in a stream.
			strh_aud.dwQuality = -1; // encoding quality given by an integer
										// between
										// 0 and 10,000. If set to -1, drivers
										// use the default
										// quality value.
			...
			wavfmt.nAvgBytesPerSec = m_iBytesPerSample * m_iSamplesPerSecond
					* m_iChannels;
	...
	private final static void writeString(RandomAccessFile raFile, String s)
			throws IOException {
		byte[] bytes = s.getBytes("UTF8");
		raFile.write(bytes);
	}

	private final static void write32LE(RandomAccessFile raFile, int v)
			throws IOException {
		raFile.write(v & 0xFF);
	...
	private static int read32LE(byte[] ab, int iPos) {
		return (ab[iPos + 0]) | (ab[iPos + 1] << 8) | (ab[iPos + 2] << 16)
				| (ab[iPos + 3] << 24);
	}
	...
	private static void JPEG2MJPEG(byte[] ab) throws IOException {
		if (ab[6] != 'J' || ab[7] != 'F' || ab[8] != 'I' || ab[9] != 'F')
			throw new IOException(
					"JFIF header not found in jpeg data, unable to write frame to AVI.");
		...
		/*
		 * ab[6] = 'A'; ab[7] = 'V'; ab[8] = 'I'; ab[9] = '1';
		 */
	}

	/**
	 * Represents an AVI 'chunk'. When created, it saves the current position in
	 * the AVI RandomAccessFile. When endChunk() is called, it temporarily jumps
	 * back to the start of the chunk and records how many bytes have been
	 * written.
	 */
	private static class Chunk {
		...
		Chunk(RandomAccessFile oRAF, String sChunkName, String sSubChunkName)
				throws IOException {
		...
		/**
		 * Jumps back to saved position in the RandomAccessFile and writes how
		 * many bytes have passed since the position was saved, then returns to
		...
		public void endChunk(RandomAccessFile oRAF) throws IOException {
			...
			m_lngSize = (lngCurPos - (m_lngPos + 4)); // save number of bytes
														// since start of chunk
			write32LE(oRAF, (int) m_lngSize); // write the header size
			oRAF.seek(lngCurPos); // return to current position
		}

		/**
		 * After endChunk() has been called, returns the size that was written.
		 */
		...
		/**
		 * Returns the position where the size will be written when endChunk()
		 * is called.
		 */
