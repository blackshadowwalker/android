...
public class uploadServlet extends HttpServlet{  
	...
	{    
		...
		//此处获取不到POST中的表单数据
		/*
		System.out.println("------- 存入 session，供其他 servlet 取数据 ----------");
		//存入 session，供其他 servlet 取数据
		Enumeration reqmap = request.getParameterNames();
		...
		if(ServletFileUpload.isMultipartContent(request)){
			try {    
				...
				System.out.println("fileListSize="+itemList.size());
				while (it.hasNext()) {    
					...
					if (item.isFormField()) { 
						...
						System.out.println("SET SESSION: "+name+"="+value);
						request.getSession().setAttribute(name, value);//将表单数据存入SESSION，供接下来的保存servlet使用
					} 
		...
		request.getSession().setAttribute("filejson", jsonmsg);
